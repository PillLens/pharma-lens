...
const DraggableFAB: React.FC<DraggableFABProps> = ({
  ...
  const PRESS_DURATION = 800; // 800ms press to start dragging
  ...
  const handlePressEnd = useCallback(() => {
    ...
    if (!isDragging && isPressed && totalMovement < MOVEMENT_THRESHOLD) {
      // It was a regular click with minimal movement - call onClick
      onClick();
    }
    ...
    setIsDragging(false);
    setIsPressed(false);
  }, [isDragging, isPressed, onClick, position, findNearestSnapZone, constrainPosition, savePosition, dragStart]);
